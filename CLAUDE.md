...
- **Hungarian vs greedy offense — tentative resolution in favor of greedy.** v1.5G (greedy + defense) latest reading ~800 μ vs v1.5 (Hungarian + defense) prior readings ~600-655 μ. The ~100 μ delta is at the edge of plausible ladder noise; keep treating the result as tentative, but the working hypothesis is now that Hungarian was the regression and `use_hungarian_offense=False` should remain the default.
